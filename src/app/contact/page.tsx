...
export default function ContactPage() {
  ...
  return (
    <div className="bg-[#FBF9F5] min-h-screen">
      ...
      {/* Quick FAQ Section */}
      <section className="py-16 bg-[#FFFFFF] border-t border-[#EBE3D7]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center space-y-2 mb-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#F5EFE6] text-xs font-semibold text-[#B86B35]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {faqs.map((faq) => (
              <div
                key={faq.q}
                className="p-6 rounded-2xl bg-[#FBF9F5] border border-[#EBE3D7] space-y-2"
              >
                <h4 className="font-heading font-bold text-sm text-[#1C1510] flex items-start gap-2">
                  <span className="text-[#B86B35] font-black">Q:</span>
                  <span>{faq.q}</span>
                </h4>
                <p className="text-xs sm:text-sm text-[#635347] leading-relaxed pl-5">
                  {faq.a}
                </p>
              </div>
            ))}
